...
/**
 * Created by Shekahar.Shrivastava on 07-Feb-18.
 */

public class PictureFIlterAdapter extends BaseAdapter {
    ...
    public PictureFIlterAdapter(Context p_context, List<String> p_item, List<String> p_path, Boolean p_isRoot) {
        m_context = p_context;
        m_item = p_item;
        m_path = p_path;
    ...
    public View getView(final int p_position, View p_convertView, ViewGroup p_parent) {
        View m_view = null;
        PictureFIlterAdapter.ViewHolder m_viewHolder = null;
        if (p_convertView == null) {
            LayoutInflater m_inflater = LayoutInflater.from(m_context);
            m_view = m_inflater.inflate(R.layout.pictures_layout, null);
            m_viewHolder = new PictureFIlterAdapter.ViewHolder();
            m_viewHolder.m_tvFileName = (TextView) m_view.findViewById(R.id.GVTextView);
            m_viewHolder.m_ivIcon = (ImageView) m_view.findViewById(R.id.GVimageVIew);
            m_view.setTag(m_viewHolder);
        } else {
            m_view = p_convertView;
            m_viewHolder = ((PictureFIlterAdapter.ViewHolder) m_view.getTag());
        }
